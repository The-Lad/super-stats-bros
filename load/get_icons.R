...
files = paste0('pics/icons/', str_extract(icons, '[^/]+$'))

if (!dir.exists('pics')) dir.create('pics')
if (!dir.exists('pics/icons')) dir.create('pics/icons')
if (any(!file.exists(files))) {
  sapply(paste0('http://www.ssbwiki.com/', icons[!file.exists(files)]), function(x) download_html(x, paste0('pics/icons/', str_extract(x, '[^/]+$'))))
}

names(files) = str_remove_all(files, 'pics/icons/[120px-]*|HeadSSBU[Website]*.png') %>% str_replace_all("([a-z])([A-Z])", "\\1 \\2") %>% str_replace_all(mapped_names)
